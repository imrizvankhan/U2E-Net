...
def lowlight_train(lowlight_enhance):
    ...
    train_low_data_names = glob('..*.png') 
    train_low_data_names.sort()
    train_high_data_names = glob('..*.png') 
    ...
    eval_low_data_name = glob('/low/*.*')

    for idx in range(len(eval_low_data_name)):
        eval_low_im = load_images(eval_low_data_name[idx])
        eval_low_data.append(eval_low_im)

    lowlight_enhance.train(train_low_data,train_low_data_eq, eval_low_data,train_high_data, batch_size=args.batch_size, patch_size=args.patch_size, epoch=args.epoch, lr=lr, sample_dir=args.sample_dir, ckpt_dir=os.path.join(args.ckpt_dir, 'Divide'), eval_every_epoch=args.eval_every_epoch, train_phase="Divide")

    # lowlight_enhance.train(train_low_data, train_high_data, eval_low_data, batch_size=args.batch_size, patch_size=args.patch_size, epoch=args.epoch, lr=lr, sample_dir=args.sample_dir, ckpt_dir=os.path.join(args.ckpt_dir, 'glitter'), eval_every_epoch=args.eval_every_epoch, train_phase="glitter")
...
def lowlight_test(lowlight_enhance):
    ...
    lowlight_enhance.test(test_low_data, test_high_data, test_low_data_name, save_dir=args.save_dir, Divide_flag=args.Divide)
